...
import com.platform.information.sms.SMSFactory;
...
//        String arr[] = result.split(",");
        String resutl = SMSFactory.build().sendSms("SMS_150175139",phone,"{'code':'"+sms_code+"'}");
    ...
    @PostMapping("getBindMobile")
    public Object getBindMobile(@LoginUser UserVo loginUser) {
        UserVo userVo = userService.queryObject(loginUser.getUserId());
        return toResponsSuccess(userVo.getMobile()==null?"":userVo.getMobile());
    }
    ...
    public Object getUserInfo(@LoginUser UserVo loginUser) {
        Map result = userService.getUserInfo(loginUser.getUserId());
