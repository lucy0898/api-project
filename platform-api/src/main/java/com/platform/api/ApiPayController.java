...
public class ApiPayController extends ApiBaseAction {
    ...
    public Object payPrepay(@LoginUser UserVo loginUser, Integer orderId) {
        ...
        if (null == orderInfo||orderInfo.getOrder_status()==101) {
        ...
        if(StringUtils.isNotBlank(orderInfo.getPay_id())){
            resultObj.put("appId", ResourceUtil.getConfigByName("wx.appId"));
            resultObj.put("timeStamp", DateUtils.timeToStr(System.currentTimeMillis() / 1000, DateUtils.DATE_TIME_PATTERN));
            resultObj.put("nonceStr", nonceStr);
            resultObj.put("package", "prepay_id=" + orderInfo.getPay_id());
     ...
     * 未登录状态下获取支付的请求参数
     */
    @ApiOperation(value = "未登录状态下获取支付的请求参数")
    @IgnoreAuth
    @PostMapping("unloginprepay")
    public Object payPrepayWithoutLogin(Integer orderId,String token) {
        String trade_type = "NATIVE";
        String spbill_create_ip = "47.93.255.124";
        String notify_url="https://wx.dongfanghongma.com/platform/api/pay/notify";
        TokenEntity tokenEntity = tokenService.queryByToken(token);
        if(tokenEntity==null){
            return  toResponsObject(99,"Token非法","");
        }
        OrderVo orderInfo = orderService.queryObject(orderId);
        if (null == orderInfo) {
            return toResponsObject(400, "订单号未找到", "");
        }
        ...
        try {
            ...
            if (null != orderGoods) {
                String body = "超市-";
                body += orderGoods.size()>0?orderGoods.get(0).getGoods_name():"";
                // 商品描述
                parame.put("body", body);
            }
            //支付金额
            ...
            // 交易类型APP
            parame.put("trade_type", trade_type);
             //getClientIp();
            parame.put("spbill_create_ip",spbill_create_ip);
            // 数字签证
            String sign = WechatUtil.arraySign(parame, ResourceUtil.getConfigByName("wx.paySignKey"));
            parame.put("sign", sign);
            String xml = MapUtils.convertMap2Xml(parame);
            ...
            if (return_code.equalsIgnoreCase("FAIL")) {
                ...
                if (result_code.equalsIgnoreCase("FAIL")) {
                    ...
                    String prepay_id = MapUtils.getString("prepay_id", resultUn);
                    String code_url = MapUtils.getString("code_url",resultUn);
                    // 先生成paySign 参考https://pay.weixin.qq.com/wiki/doc/api/wxa/wxa_api.php?chapter=7_7&index=5
                    resultObj.put("appId", ResourceUtil.getConfigByName("wx.appId"));
                    resultObj.put("timeStamp", DateUtils.timeToStr(System.currentTimeMillis() / 1000, DateUtils.DATE_TIME_PATTERN));
                    resultObj.put("nonceStr", nonceStr);
                    resultObj.put("package", "prepay_id=" + prepay_id);
                    resultObj.put("signType", "MD5");
                    resultObj.put("code_url",code_url);
                    String paySign = WechatUtil.arraySign(resultObj, ResourceUtil.getConfigByName("wx.paySignKey"));
                    resultObj.put("paySign", paySign);
                    // 业务处理
                    orderInfo.setPay_id(prepay_id);
...
//                orderInfo.getActual_price().doubleValue(), orderInfo.getActual_price().doubleValue());
     ...
     */
    @ApiOperation(value = "获取支付的请求参数")
    @PostMapping("paymemberPrepay")
    public Object paymemberPrepay(@LoginUser UserVo loginUser, Integer orderId) {
        MemberOrderVo orderInfo = memberOrderService.queryObject(orderId);
        if (null == orderInfo) {
            return toResponsObject(400, "订单已取消", "");
        }
        ...
        try {
            ...
            // 回调地址
            parame.put("notify_url", ResourceUtil.getConfigByName("wx.memberNotifyUrl"));
            // 交易类型APP
            parame.put("trade_type", ResourceUtil.getConfigByName("wx.tradeType"));
            parame.put("spbill_create_ip", getClientIp());
            parame.put("openid", loginUser.getWeixin_openid());
            ...
            if (return_code.equalsIgnoreCase("FAIL")) {
                ...
                if (result_code.equalsIgnoreCase("FAIL")) {
                    ...
                    memberOrderService.update(orderInfo);
                    return toResponsObject(0, "微信统一订单下单成功", resultObj);
                }
            }
        } catch (Exception e) {
    ...
    public Object orderMemberQuery(@LoginUser UserVo loginUser, Integer orderId) {
        if (orderId == null) {
            return toResponsFail("订单不存在");
        }
        MemberOrderVo order = memberOrderService.queryObject(orderId);
        Map<Object, Object> parame = new TreeMap<Object, Object>();
        parame.put("appid", ResourceUtil.getConfigByName("wx.appId"));
        // 商家账号。
        parame.put("mch_id", ResourceUtil.getConfigByName("wx.mchId"));
        ...
        if ("SUCCESS".equals(trade_state)) {
            // 更改订单状态
            // 业务处理
            MemberOrderVo orderInfo = new MemberOrderVo();
            orderInfo.setId(orderId);
            orderInfo.setPayStatus(2);
            orderInfo.setCardId(order.getCardId());
            orderInfo.setPayTime(new Date());
            int i = memberOrderService.update(orderInfo);
            if(i!=0){
                userService.inspireMember(order);
            }
            return toResponsMsgSuccess("支付成功");
        } else if ("USERPAYING".equals(trade_state)) {
            // 重新查询 正在支付中
     ...
     */
    @ApiOperation(value = "微信订单回调接口")
    @RequestMapping(value = "/notifyMember", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public void notifyMember(HttpServletRequest request, HttpServletResponse response) {
        try {
            request.setCharacterEncoding("UTF-8");
            response.setCharacterEncoding("UTF-8");
            response.setContentType("text/html;charset=UTF-8");
            ...
            if (result_code.equalsIgnoreCase("FAIL")) {
                ...
                logger.error("订单" + out_trade_no + "支付成功");
                // 业务处理
                MemberOrderVo orderInfo = memberOrderService.queryObject(Integer.valueOf(out_trade_no));
                orderInfo.setPayStatus(2);
                int i = memberOrderService.update(orderInfo);
                if(i!=0){
                   userService.inspireMember(orderInfo);
                }
                response.getWriter().write(setXml("SUCCESS", "OK"));
            }
        } catch (Exception e) {
