...
public class ApiUserService {
    ...
    public void inspireMember(MemberOrderVo orderInfo) {

        UserVo userVo = userDao.queryObject(orderInfo.getUserId());
        userVo.setMember_card_id(orderInfo.getCardId());
        userVo.setIs_member(1);

        MemberCardVo cardVo = memberCardService.queryObject(orderInfo.getCardId());

        userVo.setInspire_time(new Date());
        userVo.setExpire_time(memberCardService.getExpireTime(userVo.getInspire_time(),cardVo.getDescription()));
        userDao.update(userVo);

    }

    public UserVo queryByUnionId(String union_id) {
        return userDao.queryByUnionId(union_id);
    }
}
