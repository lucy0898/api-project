...
public class WechatUtil {
     ...
     * @return
     */
    public static String sendWxMsg(String openid,String accessToken,JSONObject data,String formId,String template_id) {
        String xmlStr = null;
        logger.info("*******sendWxMsg params：openid" + openid+" :"+" accessToken:"+accessToken+" data:"+data.toString()+" formId:"+formId+" template_id"+template_id);
        Map<String, String> parame = new HashMap<>();
        parame.put("appid", ResourceUtil.getConfigByName("wx.appId"));
        parame.put("grant_type", "client_credential");
        parame.put("secret", ResourceUtil.getConfigByName("wx.secret"));
        if(org.apache.commons.lang.StringUtils.isBlank(accessToken)){
            Map<String,Object> access_token = (Map<String,Object>)JSONObject.parse(HttpUtil.URLGet("https://api.weixin.qq.com/cgi-bin/token", parame, "UTF-8"));//获取accesstoken
            accessToken =  access_token.get("access_token").toString();
        }

        JSONObject outputStr = new JSONObject();
        outputStr.put("touser",openid);
        outputStr.put("template_id",template_id);
        outputStr.put("page","index");
        outputStr.put("form_id",formId);

        outputStr.put("data",data);
//        outputStr.put("form_id","AT0009");
        String msgUrl = ResourceUtil.getConfigByName("wx.sendMsg").replace("ACCESS_TOKEN",accessToken);
        HttpPost httPost = new HttpPost(msgUrl);
        httPost.addHeader("Connection", "keep-alive");
        httPost.addHeader("Accept", "*/*");
        httPost.addHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
        httPost.addHeader("Host", "api.mch.weixin.qq.com");
        ...
        httPost.addHeader("Cache-Control", "max-age=0");
        httPost.addHeader("User-Agent", "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0) ");
        httPost.setEntity(new StringEntity(outputStr.toString(), "UTF-8"));
        BasicHttpClientConnectionManager connManager;
        ...
        try {
            ...
            JSONObject result = JSONObject.parseObject(xmlStr);
            if(result.get("errcode").equals(0)){
                return "success";
            }
            logger.info("*******sendWxMsg result：" + xmlStr);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return null;
        }
    ...
    public static String getAccessToken(){
        ...
        if(J2CacheUtils.get("wx", "access_token")==null||"".equals(J2CacheUtils.get("wx", "access_token"))){
            Map<String, String> parame = new HashMap<>();
            parame.put("appid", ResourceUtil.getConfigByName("wx.appId"));
            parame.put("grant_type", "client_credential");
            parame.put("secret", ResourceUtil.getConfigByName("wx.secret"));
            Map<String, Object> access_token = (Map<String, Object>) JSONObject.parse(HttpUtil.URLGet("https://api.weixin.qq.com/cgi-bin/token", parame, "UTF-8"));//获取accesstoken
            accessToken = access_token.get("access_token").toString();
            J2CacheUtils.put("wx", "access_token", access_token.get("access_token").toString());
            logger.info("util  memory token " + access_token.toString());
        }else {
            return J2CacheUtils.get("wx", "access_token").toString();
        }
